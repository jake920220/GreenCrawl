...
var __data = require('./lib/dataHelper');

var __dbHelper = require('./lib/dbHelper');
__dbHelper.initDB('localhost/greenDB');
var green = __dbHelper.getDB('test');

var RegGetModelNum = /[0-9]{2,3}/gi;
var RegKorCheck = /[ㄱ-ㅎ|ㅏ-ㅣ|가-힣]/;
var changeExp = /[\r\n\s!@#$&%^*()\=+\\\|\[\]{};:\'`"~,.<>\/?]/g;

var modelTonModelName = __data.modelTonModelName;
var modelTonFilter = __data.modelTonFilter;
var pageName = __data.pageName; //페이지에서 아이디를 따올때
var idPage = __data.idPage;     //게시물에서 내용따올때
var imgLink = __data.imgLink;   //이미지 따오는 경로
var CategoryFirst = __data.partCategoryFirst;
var thirdCategory = __data.categorys;
var secondCategory = __data.secondCategory;
var changeKeyword = __data.searchKeyword;
...
(function () {
    ...
    output.onePage = function (id, category) {
        var options = {
        ...
        request(options, function (err, res, html) {
            ...
            for (var i = 0; i < resultArr.length; i++) {
                if (resultArr[i]['modelTon'] == '' || typeof resultArr[i]['modelTon'] == 'undefined' || resultArr[i]['modelTon'] == null ||
                    resultArr[i]['name'] == '' || typeof resultArr[i]['name'] == 'undefined' || resultArr[i]['name'] == null ||
                    resultArr[i]['partCategorySecond'] == null || typeof resultArr[i]['partCategorySecond'] == 'undefined' || resultArr[i]['partCategorySecond'] == '') {
                    failedArr.push(resultArr[i]);
    ...
    output.getCSVSeperated = function (startPages, maxPages, category) {
        ...
        var yesterday2 = new Date(yesterday.getTime() - 24 * 60 * 60 * 1000);
        var yesterday3 = new Date(yesterday2.getTime() - 24 * 60 * 60 * 1000);
        var nowDate = [];
        if (date.getDay() == 1) {
            console.log("it's monday, crawling starts friday to sunday");
            nonwDate = [];
            nowDate.push(( ('0' + (date.getMonth() + 1)).slice(-2) + '/' + ('0' + yesterday.getDate()).slice(-2)));
            nowDate.push(( ('0' + (date.getMonth() + 1)).slice(-2) + '/' + ('0' + yesterday2.getDate()).slice(-2)));
            nowDate.push(( ('0' + (date.getMonth() + 1)).slice(-2) + '/' + ('0' + yesterday3.getDate()).slice(-2)));
        } else {
            nowDate.push(( ('0' + (date.getMonth() + 1)).slice(-2) + '/' + ('0' + yesterday.getDate()).slice(-2)));
        }
        ...
        async.whilst(
            function () {
                    ...
                    function (next) {
                        ...
                        request(options, function (err, res, html) {
                            ...
                            bigForm.each(function () {
                                var obj = {};
                                var writtenDate = $(this).find('td').eq(12).toString();
                                var urlLink = $(this).find('td').eq(3).toString();

                                var start = writtenDate.indexOf(';' + date.getFullYear()) + 5;
                                var end = writtenDate.indexOf('&#xA0;', start + 1);
                                var str = writtenDate.substring(start + 1, end);


                                var urlStart = urlLink.indexOf(';id=') + 3;
                                var urlEnd = urlLink.indexOf('&amp;page');
                                var url = urlLink.substring(urlStart + 1, urlEnd);

                                obj[str] = url;

                                if (nowDate.indexOf(Object.keys(obj)[0]) != -1) {
                                    arr.push(obj[str]);
                                } else {
                                    console.log("remove not matching date", obj[str]);
                                }
                            })
                ...
                resultArr.sort(sortBy('가격', '모델명'));

                var deletedCount = 0;
                for (var i = 0; i < resultArr.length - 1; i++) {
                    if (resultArr[i].모델명 == resultArr[i + 1].모델명 && resultArr[i].가격 == resultArr[i + 1].가격) {
                ...
                async.waterfall([
                    function (next) {
                        //DB에서 하나하나 비교해서 있는지 없는지 확인한다
                        var index = 0;
                        console.log("hi");
                        async.each(resultArr, function (obj, cb) {
                            green.findOne({
                                name: obj.name,
                                ...
                                if (data != null) {
                                    needToRemoveDB.push(data.folder);
                                    needToRemoveCSV.push(obj.folder);
                                    index++;
                                    cb();
                                } else {
                                    index++;
                            ...
                            next(null, needToRemoveDB);
                        });
                    },
                    function (removeArr, next) {
                        //아까 디비내에서 삭제해야할 애들의 ID값이 담긴 어레이를 기준으로 돌면서 다지운다.
                        if (removeArr.length > 0) {
                            async.each(removeArr, function (key, cb) {
                                if (typeof key != "undefined" || key != null) {
                                    green.remove({key: key}, function () {
                                        cb();
                                    });
                                } else {
                                    console.log("WTF", key);
                            ...
                            console.log("DB내에 중복된 것이 없다.");

                            next(null);
                        }
                    },
                    function (next) {
                        //디비내에서는 중복된애들이 지워진상태이고, 새거는 INSERT시킨다.
                        if (resultArr.length != 0) {
                            if (resultArr.length == 1) {
                                green.insert(resultArr[0], function (err, data) {
                                    if (err) {
                                        console.log(err);
                                        return;
                                    }
                                ...
                                green.insertMany(resultArr, function (err, data) {
                                    if (err) {
                                        console.log(err);
                                    }
                                    console.log("데이터넣기");
                    ...
                    function (next) {
                        ...
                        var failedArr = [];
                        for (var i = 0; i < resultArr.length; i++) {
                            if (needToRemoveCSV.indexOf(resultArr[i].folder) != -1) {
                                resultArr.splice(i, 1);
                            }
                        }

                        for(var i=0; i<resultArr.length; i++){
                            objRemoving(resultArr[i]);
                            if (resultArr[i]['modelTon'] == '' || typeof resultArr[i]['modelTon'] == 'undefined' || resultArr[i]['modelTon'] == null ||
                                resultArr[i]['name'] == '' || typeof resultArr[i]['name'] == 'undefined' || resultArr[i]['name'] == null ||
                                resultArr[i]['partCategorySecond'] == null || typeof resultArr[i]['partCategorySecond'] == 'undefined' || resultArr[i]['partCategorySecond'] == '') {
                                failedArr.push(resultArr[i]);
                    ...
                    if (failedArr.length > 0) {
                        async.waterfall([
                            function (next) {
                                ...
                                fs.writeFile(category + '/' + theName + '.csv', csv, function (err) {
                                    if (err) {
                                        console.log(err);
                                        next(err);
                                        return;
                            ...
                            function (next) {
                                ...
                                var date = new Date();
                                var theName = category + '_failed' + '_' + (date.getMonth() + 1) + '_' + (date.getDate() - 1);
                                fs.writeFile(category + '/' + theName + '.csv', csv, function (err) {
                                    if (err) {
                                        console.log(err);
                                        next(err);
                        ...
                        fs.writeFile(category + '/' + theName + '.csv', csv, function (err) {
                            ...
                            console.log("FINISH WITH NO FAILED");
                        });
                    }
    ...
    output.failedImgDown = function (failed, category) {
        async.waterfall([
            ...
            function (objArr, next) {
                ...
                if (!fs.existsSync(category + '/' + theName)) {
                    fs.mkdirSync(category + '/' + theName);
                }

                for (var i = 0; i < objlen; i++) {
                    if (fs.existsSync(category + '/' + theName + '/' + objArr[i].key)) {
                        continue;
                    }
                    fs.mkdirSync(category + '/' + theName + '/' + objArr[i].key);
                }
                ...
                async.each(objArr, function (obj, cb) {
                    async.each(obj.imgs, function (img, callback) {
                        var options = {
                            ...
                            dest: category + '/' + theName + '/' + obj.key,
                            done: function (err, filename, image) {
                                if (err) {
                                    console.log("==========================");
                                    console.log(obj.key);
                                    console.log("==========================");
                                    theError = theError + obj.key + ',';
                                }
    ...
    output.getImgs = function (fileName, category) {
        ...
        async.waterfall([
            ...
            function (objArr, next) {
                var date = new Date();
                var theName = fileName.slice(0,-4);
                console.log("##@#@#@#@#@#@#@#@#@#@",theName);
                ...
                for (var i = 0; i < objlen; i++) {
                    if (fs.existsSync(theName + '/' + objArr[i].key)) {
                        continue;
                    }
                    fs.mkdirSync(theName + '/' + objArr[i].key);
                }
...
function objRefactoring(obj) {
    ...
    var goToSecondCategory = true;
    var goToNumberModelTon = true;
    ...
    var filteredModelName = obj['모델명'].replace(changeExp, '');
    filteredModelName = filteredModelName.replace(/[ㄱ-ㅎ|ㅏ-ㅣ|가-힣]/gi, '').toUpperCase();

    for (item in modelTonModelName) {
        if (filteredModelName == item) {
            obj['modelTon'] = modelTonModelName[item];
            goToNumberModelTon = false;
            break;
        }
    }
    if (goToNumberModelTon == true) {
        if (typeof modelTonFilter[num] != 'undefined') {
            obj['modelTon'] = modelTonFilter[num];
        } else if (obj['모델명'].indexOf('텐') != -1) {
            obj['modelTon'] = modelTonFilter["10"];
        }
    }
    ...
    obj['priceCustomer'] = Math.floor((parseInt(obj['priceFinal'] / 0.85)) / 10000) * 10000;

    if (obj['모델명'].indexOf('신품') != -1) {
        obj['productState'] = 'productStateNew';
    } else {
        obj['productState'] = 'productStateAPlus';
    }


    for (item in thirdCategory) {
        if (obj['모델명'].indexOf(item) != -1) {
            var arr = thirdCategory[item].split('/');
            obj['partCategorySecond'] = arr[0];
            obj['name'] = arr[0];
            if (arr.length == 2) {
                obj['partCategoryThird'] = arr[1];
                obj['name'] = arr[1];
            }
            goToSecondCategory = false;
            break;
        }
    }

    if (goToSecondCategory) {
        for (item in secondCategory) {
            if (obj['모델명'].indexOf(item) != -1) {
                var arr = secondCategory[item].split('/');
                obj['partCategorySecond'] = arr[0];
                obj['name'] = arr[0];
                if (arr.length == 2) {
                    obj['partCategoryThird'] = arr[1];
    ...
    if (obj.name == null || obj.name == '' || typeof obj.name == 'undefined' || obj.name.length == 0) {
        //네임이 없을때 모델명이 디스플레이네임으로 들어간다.
        obj['displayName'] = obj['모델명'];
    } else {
        //네임이 있을때.
        var total = 0;
        ...
        if (changeKeyword[obj.name] == -1 || typeof changeKeyword[obj.name] == 'undefined') {
            obj['displayName'] = obj['모델명'];
        } else {
            for (item in changeKeyword[obj.name]) {
                total += changeKeyword[obj.name][item];
            }
            for (item in changeKeyword[obj.name]) {
                count += parseInt((((changeKeyword[obj.name][item]) / total) * 10000).toFixed(0));
                percent[item] = count;
            }
            var random = Math.floor(Math.random() * 10000) + 1;

            for (item in percent) {
                if (percent[item] > random) {
                    obj['displayName'] = num + ' ' + item;
                    break;
                }
            }
        }
